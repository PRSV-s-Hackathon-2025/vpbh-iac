...
{
  "connectorName": "vpbh-bigdata-cluster-kinesis-sink",
  "connectorConfiguration": {
    ...
    "kafka.sasl.jaas.config": "org.apache.kafka.common.security.plain.PlainLoginModule required username=\"D7CGBNHBWIMUP4VJ\" password=\"XdyfB3gBynCAB4ejr4BL1peQybQNb02DMIXqS/6hI2WjyI9CGoRxJb+stceObh82\";",
...
EXISTING_CONNECTOR=$(aws kafkaconnect list-connectors --region $REGION --query "connectors[?connectorName=='$CONNECTOR_NAME'].connectorName" --output text)
if [ ! -z "$EXISTING_CONNECTOR" ]; then
  echo "Deleting existing connector: $EXISTING_CONNECTOR"
  aws kafkaconnect delete-connector --connector-name $CONNECTOR_NAME --region $REGION
